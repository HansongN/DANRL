...
def scholar_fos(sid):
    rootpath = "data/seed scholars collaborators papers"
    filename = sid + ".txt"
    ...
    if len(title_year_fos) > 0:
        ...
        temp["fos"] = year_fos
        # print("写入文件")
        out = open("data/seed_scholars_collaborators_fos.txt", "a")
...
if __name__ == '__main__':
    ...
    print("读入id name")
    sid_name = read_seed_scholar_collaborator_id_name()
    read_sid = list()
    with open("data/seed_scholars_collaborators_fos.txt", "r") as scholars:
        for scholar in scholars:
            scholar_json = json.loads(scholar)
            read_sid.append(scholar_json["id"])
    scholars.close()
    print("分析")
    i = 0
    for sid, sname in sid_name.items():
        i += 1
        if sid not in read_sid:
            scholar_fos(sid)
